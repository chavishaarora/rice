  ...
  import {
      ...
      ShoppingCart,
      Trees,
      CheckCircle, // Added for selection icon
  } from "lucide-react";

  interface Suggestion {
      // Note: Changed id from string to number to match backend DB
      id: string; 
      type: "flight" | "hotel" | "attraction" | "restaurant" | "shop" | "leisure";
      title: string;
      description: string;
      price: number;
      rating: number;
  ...
  interface SuggestionsPanelProps {
      ...
      refreshKey: number;
      // We'll use this to tell the parent component (the chat) to re-render
      // the chat history after a selection is made (optional, but good practice)
  ...
  const SuggestionsPanel = ({
      ...
      const loadSuggestions = async () => {
          ...
          try {
              ...
              // Ensure ID is converted to number since it's an integer in the DB
              const loadedSuggestions = (data.suggestions as any[]).map(s => ({
                ...s,
                id: s.id.toString(), // Ensure it is treated as a string, though often redundant if DB returns string
              })) as Suggestion[];
      ...
      const API_BASE_URL = "http://localhost:5001/api"; // Define the base API URL
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/select_suggestion/${conversationId}`,
                {
                    ...
                    credentials: "include",
                    // The backend should handle the raw string ID now
      ...
      return (
          <div className="p-4 space-y-6 h-full overflow-y-auto">
              ...
              {/* --- Render Flights (UPDATED with Select Button) --- */}
              {suggestions.flights.length > 0 && (
                  <section>
                      <h2 className="text-2xl font-bold mb-4 flex items-center">
                          <Plane className="mr-2" /> Flights
                          {/* Show checkmark if only ONE flight is left (finalized) */}
              ...
              {/* --- Render Hotels (UPDATED with Select Button) --- */}
              {suggestions.hotels.length > 0 && (
                  <section>
                      <h2 className="text-2xl font-bold mb-4 flex items-center">
                          <Hotel className="mr-2" /> Hotels
                          {/* Show checkmark if only ONE hotel is left (finalized) */}
